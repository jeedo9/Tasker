...
export type Error400ReturnType = {status: Code.BadRequest, errors: ZodIssue[]} & ErrorSuccess
...
export function error(status: Code.BadRequest, errors: ZodIssue[]): Error400ReturnType
export function error<K extends CRUD>(status: Exclude<CRUDAccess<K, 'error'>, Code.BadRequest>, message: string): ErrorType<K>

export function error(status: AllErrorCodes, errorsOrMessage: ZodIssue[] | string) {
    return {
    status,
